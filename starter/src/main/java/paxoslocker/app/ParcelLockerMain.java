...
public final class ParcelLockerMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int holdSeconds = integer(options, "run-seconds", 0);
        new ClusterOptions(f, acceptorCount);
        ...
        RuntimeNodeFactory factory = new DefaultRuntimeNodeFactory();
        var workers = new DefaultWorkerFactory();
        ...
        try (var transport = new LocalTcpTransport()) {
            try {
                ...
                List<NodeLifecycle> reverse = new ArrayList<>(nodes);
                Collections.reverse(reverse);
                reverse.forEach(NodeLifecycle::stop);
            }
        }
    }
    ...
    private static Set<NodeId> ids(String prefix, int count) {
        if (count < 1) throw new IllegalArgumentException(prefix + " count must be positive");
        Set<NodeId> ids = new LinkedHashSet<>();
        for (int i = 0; i < count; i++) ids.add(new NodeId(prefix + "-" + i));
        return Set.copyOf(ids);
    }

    private static void usage() {
        System.out.println("Usage: --fault-tolerance <f> [--acceptors n] [--replicas n] [--leaders n] [--data-dir path] [--run-seconds n]");
    }
}
