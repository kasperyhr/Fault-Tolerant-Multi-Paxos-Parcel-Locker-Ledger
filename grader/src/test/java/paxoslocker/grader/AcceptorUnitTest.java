...
@Tag("student") class AcceptorUnitTest {
 ...
 @Test void firstP1aIsAdopted(){try(var t=UnitFixtures.transport()){var a=node(t);var b=new BallotNumber(1,"L1");var response=a.onP1a(new P1aMessage(b));assertEquals(b,response.requestedBallot());assertEquals(b,response.acceptorBallot());}}
 @Test void lowerP1aDoesNotRegress(){try(var t=UnitFixtures.transport()){var a=node(t);var high=new BallotNumber(2,"L1");a.onP1a(new P1aMessage(high));assertEquals(high,a.onP1a(new P1aMessage(new BallotNumber(1,"L1"))).acceptorBallot());}}
 ...
 @Test void lowerP2aIsRejected(){try(var t=UnitFixtures.transport()){var a=node(t);a.onP1a(new P1aMessage(new BallotNumber(2,"L1")));var requested=new BallotNumber(1,"L1");var response=a.onP2a(new P2aMessage(new PValue(requested,1,new NoOp(UUID.randomUUID()))));assertEquals(requested,response.requestedBallot());assertEquals(new BallotNumber(2,"L1"),response.acceptorBallot());}}
